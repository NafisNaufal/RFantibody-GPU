...
resume_rf2() {
    ...
    TOTAL=$(qvls "$DIR/2_proteinmpnn.qv" | wc -l)
    DONE=$(qvls "$DIR/3_rf2.qv" | wc -l)
    ...
    qvls "$DIR/3_rf2.qv"          | sort > /tmp/rf2_done.txt
    qvls "$DIR/2_proteinmpnn.qv"  | sort > /tmp/rf2_all.txt
    ...
    cat /tmp/rf2_remaining.txt | qvslice "$DIR/2_proteinmpnn.qv" > "$DIR/2_mpnn_remaining.qv"
